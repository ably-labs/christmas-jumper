...
describe("ActiveImageSelector", () => {
    ...
    it("can return snaked frames from valid detected image", async () => {
       ...
       const result = await sut.getFrames(song);

       expect(JSON.stringify(result.rgbFrames[0])).toBe(JSON.stringify(expectation));
    });

    it("can get multiple frames when key has more than one image", async () => {
        const result = await sut.getFramesForKey("frametest");
